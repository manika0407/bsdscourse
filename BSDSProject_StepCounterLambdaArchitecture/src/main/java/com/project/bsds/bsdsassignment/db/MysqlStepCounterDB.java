...
public class MysqlStepCounterDB implements IStepCounterDB {
  private static final boolean REMOTE_DB = true; // use remote rds db or local mysql db
  private static final boolean RDS_DB = false; // if remote db use rds db or google cloud db
  private static final String DB_ADDRESS =
      REMOTE_DB
          ? (RDS_DB
              ? "bsdsassignment2-stepcounter-db.cumfxxz0jzwo.us-west-2.rds.amazonaws.com" // <--- aws rds
              : "35.185.81.124") // <--- google cloud sql
          : "localhost";
  ...
  private static final Integer MAX_CONN_POOL_SIZE = 50; // keep this pool size for 5-6 ec2 instances (~300)
  private static final Integer MIN_IDLE_CONNS = 10;
  private static final Integer CONN_TIMEOUT_MS = 10 * 1000;
  private static final Integer CONN_MAX_LIFETIME_MS = 60 * 1000;
  private static final Integer CONN_IDLE_TIME_MS = 60 * 1000;
  private static final Integer ST_CACHE_SIZE = 1000;
  private static final boolean RESET_TABLE = false;

  private DataSource dataSource;

  private static MysqlStepCounterDB staticDBInstance;

  public static MysqlStepCounterDB getInstance() {
    if (staticDBInstance == null) {
      synchronized (MysqlStepCounterDB.class) {
        if (staticDBInstance == null) {
          try {
            staticDBInstance = new MysqlStepCounterDB();
          } catch (IOException e) {
  ...
  private MysqlStepCounterDB() throws IOException {
    connect();
  ...
  private DataSource getDataSource() {
    ...
    config.setMinimumIdle(MIN_IDLE_CONNS);
    config.setMaximumPoolSize(MAX_CONN_POOL_SIZE);
    config.setMaxLifetime(CONN_MAX_LIFETIME_MS);
    config.setIdleTimeout(CONN_IDLE_TIME_MS);
  ...
  private void truncateTable() throws IOException {
    ...
    try (Connection connection = dataSource.getConnection();
      ...
      throw new IOException("can't truncate table", se);
    }
  }
  ...
  public static void main(String[] args) throws IOException {
    MysqlStepCounterDB db = MysqlStepCounterDB.getInstance();
    db.truncateTable();
    db.insert(1, 1, 1, 500, null);
    db.insert(1, 1, 2, 200, null);
    db.insert(1, 2, 3, 300, null);
    db.insert(1, 2, 4, 400, null);
